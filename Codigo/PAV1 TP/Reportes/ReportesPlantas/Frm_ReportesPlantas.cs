...
namespace PAV1_TP.Reportes.ReportesPlantas
{
	public partial class Frm_ReportesPlantas : Form
	{
		...
		private void BuscarstdPlanta()
		{
			if (txt_std_MesPlanta.Text != "")
			{
				if (int.Parse(txt_std_MesPlanta.Text) > 12 | int.Parse(txt_std_MesPlanta.Text) == 0)
				{
					MessageBox.Show("Ingrese un mes correcto");
					txt_std_MesPlanta.Focus();

				}
				else
				{
					tablaPPM = factura.ReportePPM(txt_std_MesPlanta.Text);
					if (tablaPPM.Rows.Count == 0)
					{
						MessageBox.Show("No hubo ventas en el mes ingresado", "ATENCIÓN");
					}
		...
		private void ArmarEstadisticaPlanta_Torta()
		{
			...
			parametros[1] = new ReportParameter("TiempoEjecucion", _BD.FechaHora());
			rv_stdPlanta.LocalReport.SetParameters(parametros);
			rv_stdPlanta.LocalReport.DataSources.Clear();
			rv_stdPlanta.LocalReport.DataSources.Add(DatosPPM);
			rv_stdPlanta.RefreshReport();
		}

		private void ArmarEstadisticaPlanta_Barras()
		{
			Be_BaseDeDatos _BD = new Be_BaseDeDatos();

			ReportDataSource DatosPPM = new ReportDataSource("DataSet1", tablaPPM);
			rv_stdPlanta.LocalReport.ReportEmbeddedResource = "PAV1_TP.Reportes.ReportesPlantas.EstadisticaPlantasBarras.rdlc";
			ReportParameter[] parametros2 = new ReportParameter[2];
			parametros2[0] = new ReportParameter("ParametroBarrasPlanta", "Estadistica plantas vendidas en el mes: " + txt_std_MesPlanta.Text);
			parametros2[1] = new ReportParameter("TiempoEjecucion", _BD.FechaHora());
			rv_stdPlanta.LocalReport.SetParameters(parametros2);
			rv_stdPlanta.LocalReport.DataSources.Clear();
			rv_stdPlanta.LocalReport.DataSources.Add(DatosPPM);
			rv_stdPlanta.RefreshReport();
		}

		private void ArmarEstadisticaPlanta_BarrasSeries()
		{
			Be_BaseDeDatos _BD = new Be_BaseDeDatos();

			ReportDataSource DatosPPM = new ReportDataSource("DataSet1", tablaPPM);
			rv_stdPlanta.LocalReport.ReportEmbeddedResource = "PAV1_TP.Reportes.ReportesPlantas.EstadisticaPlantasBarraSeries.rdlc";
			ReportParameter[] parametros3 = new ReportParameter[2];
			parametros3[0] = new ReportParameter("ParametroBarraSeries", "Estadistica plantas vendidas en el mes: " + txt_std_MesPlanta.Text);
			parametros3[1] = new ReportParameter("TiempoEjecucion", _BD.FechaHora());
			rv_stdPlanta.LocalReport.SetParameters(parametros3);
			rv_stdPlanta.LocalReport.DataSources.Clear();
			rv_stdPlanta.LocalReport.DataSources.Add(DatosPPM);
			rv_stdPlanta.RefreshReport();
		}
		private void btn_CalcularPlanta_Click(object sender, EventArgs e)
		{
			...
			if (cmb_stdPlanta.SelectedItem.ToString() == "Gráfico de torta")
			{
				BuscarstdPlanta();
				ArmarEstadisticaPlanta_Torta();
			}
			if (cmb_stdPlanta.SelectedItem.ToString() == "Gráfico de barras (vertical)")
			{
				BuscarstdPlanta();
				ArmarEstadisticaPlanta_Barras();
			}

			if (cmb_stdPlanta.SelectedItem.ToString() == "Gráfico de barras (horizontal)")
			{
				BuscarstdPlanta();
		...
		private void rv_stdPlanta_Load(object sender, EventArgs e)
		{

		}

		private void cmb_stdPlanta_SelectedIndexChanged(object sender, EventArgs e)
		{
